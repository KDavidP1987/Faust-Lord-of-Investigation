...
> **Status (ApiVersion 19):** as of Faust 0.14.0, **every feature ships AdminOnly by default** (Faust is
> an admin tool first; admins grant pieces to players per server) — so a fresh handshake shows
> `…=admin:…` for all tokens to a non-admin. New in **19** (Faust 0.16.4): **`heatmap` gains time windows** —
> `.faust api heatmap [scope] [days] [page]` (days 0 = all-time, N = last N UTC days: today/week/month), with
> `[FAUST:hmhead]` echoing `days=`/`retentiondays=` (additive — an older Raphael omits `days` and still reads
> the all-time map; §heatmap). New in **18** (Faust 0.16.0): two new features each with its
...
### `heatmap` (player-position density) — IMPLEMENTED (ApiVersion ≥16; time windows ≥19)
...
`.faust api heatmap [<all|name|steamId>] [days] [page]` — `all`/`server` (default) = the whole server summed;
a name/SteamID = that one player. **`days` (ApiVersion ≥19, optional, default 0)** picks the time window:
`0` = all-time (cumulative, never pruned), `1` = today, `7` = this week, `30` = this month, or any N = the
last N UTC days. A header line (page 1), then **packed** cell rows (paged):
```
[FAUST:hmhead] scope=<server|steamId> [days=<n> retentiondays=<n>] cell=<f> samples=<n> cells=<n> bounds=<minCx>:<minCz>:<maxCx>:<maxCz> [mapbounds=<minCx>:<minCz>:<maxCx>:<maxCz>] collecting=<0|1>
[FAUST:hmrow] data=<cx>:<cz>:<count>,<cx>:<cz>:<count>,...          ; many cells per line, paged
...rows (densest cells first)...
[FAUST:end] cmd=heatmap scope=<...> days=<n> page=<cur>/<total> count=<totalCells>
```
...
- **Time windows (ApiVersion ≥19).** `days=0` is the cumulative all-time map (since install / last reset,
  never pruned). `days=N` sums a retention-pruned **per-UTC-day** layer over the last N days — Raphael should
  offer a today / week / month / all toggle (the same pattern as the `kills` board). `[FAUST:hmhead]` echoes
  `days=` and `retentiondays=` (the server's `[Faust.Heatmap] RetentionDays`, default 30) — a window larger
  than `retentiondays` just sums what's kept, so cap the toggle at `retentiondays`. **`days=` is additive**:
  an older Raphael omits it (→ `days=0`) and reads the all-time map exactly as before.
- **Resolution** is fixed once data exists (changing `CellSize` needs a `.faust admin data wipe heatmap`).
  Bounded by `[Faust.Heatmap] MaxCells` (applied to the all-time and per-day layers independently). An
  empty/disabled grid still sends the header + `count=0` trailer.
...
- **`status=down`** — NOT locatable on the map right now, so the live fields (`x`/`z`/`region`/`hp`/…) are
  **omitted**. Two sources: (a) a live VBlood entity with **no on-map position** by either position component
  (truly pooled / awaiting respawn — see §18b), or (b) a boss a player has defeated (from the unlock store),
  which carries `defeated=1`. **`up` rows can carry coordinates anywhere on the map, well past ±5000** (the §16
  belief that >±5000 = a bogus sentinel was disproven — see §18). **⚠️ Raphael must keep its old ±5000
  client-side coord guard raised to ~9000 (or removed)** — already DONE in Raphael v0.58.1 — or it would
  re-hide legitimate outer-region bosses Faust sends as `up`.
...
- **Single `boss <name|guid>` lookup** emits ONE `[FAUST:boss]` with **no** end trailer (commit immediately,
  ...
  still sends `count=0`.
- **Scope:** V Rising has **statically-placed** bosses (fixed lair) and **roaming** bosses (patrol a path —
  some a tiny loop, some a whole region/map). The board lists every live boss it can locate within the on-map
  cutoff `[Faust.Bosses] MapLimit` (default **9000**, covering the whole map) as `up` with coordinates, plus
  any defeated-but-not-currently-locatable boss as `down`. "dead" collapses into `down`. On-demand; zero
  passive cost. *(History: §18 the original ±5000 cutoff; §18b roaming bosses read `down` from a stale
  position component — fixed in 0.16.1 by reading `Translation`.)*
- **§18 — RESOLVED (server-side); ⚠️ needs a Raphael-side guard change.** Some bosses read `down` when the
  ...
  **⚠️ Raphael action:** the old **±5000 client-side coord guard** from §16 ("treat boss coords beyond ±5000 as
  ...
  bogus sentinel; that's no longer true.) **DONE (Raphael v0.58.1): the client guard is fully removed** —
  `FaustBoss.OnMap == HasPos`, Raphael shows whatever coords Faust sends.

- **§18b — FIXED in Faust 0.16.1 (server-side); ⏳ pending in-game confirm. No Raphael change required.**
  *(Field report 2026-06-13: `.faust api bosses` returned `count=52` split ~**25 `up` / ~27 `down`**, the `down`
  set map-WIDE — Clive/Lidia/Angram/Gorecrusher/Wolf/Wendigo/Poloma etc. — including the boss **nearest** the
  lone admin, on a no-other-players dev server. Raphael was confirmed correct: pages all rows, no coord guard,
  friendly-name map verified — so `down` came straight off the wire.)*
  - **Root cause:** the `down` set is the **roaming** bosses (V Bloods that patrol rather than sit in a fixed
    lair). The board read each boss's position from **`LocalToWorld`** — the render matrix — which goes **stale**
    for a streamed-out / pooled entity (a roamer with no player nearby keeps an old/off-map matrix value). It is
    NOT a `MapLimit`/distance problem (that's why it's map-wide and even the closest boss was affected) and NOT
    plain streaming (the closest boss was the one that broke).
  - **Fix (0.16.1):** the board now reads **`Translation`** (the sim-authoritative current position) first and
    falls back to `LocalToWorld`, taking whichever source is on-map. Roamers the server is still tracking now
    report `status=up` with their live coordinates. **No wire-shape change → ApiVersion stays 18**; Raphael needs
    no change — the same `[FAUST:boss]` rows simply carry coords for more bosses. *(A boss with NO on-map position
    by either component — genuinely pooled / awaiting respawn — still reports `down`; that's honest, not a bug.)*
  - **Verify in-game:** restart the server, confirm the handshake now reports **`plugin=0.16.1`** (proves the fix
    is deployed), then `.faust api bosses` — the previously-`down` roamers should now be `up` with coords. The
    enhanced `.faust admin bossdiag` (no filter) prints `LTW[on/off] · TR[on/off]` and an `off-LTW-but-on-TR`
    count; a non-zero count is the direct fingerprint of this bug being fixed. `.faust admin bossdiag icons`
    dumps the map-icon picture if any roamers *still* read `down` (the next fallback would be spawn-zone data).
  - **⚠️ RE-TEST RESULT (Raphael field report, 2026-06-13 — the 0.16.1 fix did NOT resolve it).** The server is now
    confirmed running the fix — handshake reports **`plugin=0.16.1`** — but `.faust api bosses` is **byte-identical
    to before: still `count=52`, ~25 `up` / ~27 `down`, the SAME roaming set down** (`CHAR_Bandit_Bomber`,
    `CHAR_Bandit_Chaosarrow`, `CHAR_Forest_Wolf`, `CHAR_Cursed_MountainBeast`, `CHAR_Wendigo`, `CHAR_Poloma`,
    `CHAR_Gloomrot_Purifier`, `CHAR_VHunter_*`, `CHAR_Villager_*`, etc.). Raphael re-verified correct: it fetches
    **all 3 pages** (`page=1/3 → 3/3`) and **accumulates all 52** before rendering, no guard, names verified — so
    the `down` rows are coming straight off the 0.16.1 wire. **So reading `Translation` first did not catch these
    bosses** — for them, `Translation` is apparently also absent / off-map (the entity is streamed out / not
    instantiated), i.e. the `off-LTW-but-on-TR` case the §B1 fix targets is NOT what's happening here. **Faust next
    step:** run `.faust admin bossdiag` on a still-down map-central boss (e.g. `CHAR_Bandit_Bomber_VBlood`) and read
    its `LTW[on/off] · TR[on/off]`: if **both off**, the entity genuinely has no live position (not instantiated) →
    neither component helps and the only path is **spawn-zone / map-icon data** (`bossdiag icons`) or accepting
    `down` as honest for an un-instantiated roamer. **No Raphael change — Raphael displays whatever Faust reports.**
  - **§18c — Faust 0.16.2: the "combine."** Acting on the re-test (Translation also off for these roamers) and the
    Raphael-side model (status from one source, location from another): the board now keeps boss **status/health/
    level from the combat entity** (present for every boss) and fills the **location from the game's own map-token
    tracking** when the combat entity is off-map — `Script_BloodAltar_TrackVBloodUnit_Shared { TrackedUnit,
    TrackPosition }` first, then the V Blood map icon (`MapIconData.TargetUser`), keyed by prefab GUID. **Still no
    wire change → ApiVersion stays 18; no Raphael change** — the same `[FAUST:boss]` rows just carry coords for the
    roamers. Confirm via handshake **`plugin=0.16.2`**, then `.faust admin bossdiag` (no filter): the new
    `Map tokens: N resolved; M rescued` line shows whether the token sources carry positions on this server (and a
    `token=(x,z)` column per boss in `bossdiag <name>`). If `N=0`, neither token source is populated passively and
    the next step is resolving icons by `NetworkId` or replicating `GetVBloodPositionsSystem`.
  - **⚠️ RE-TEST RESULT (Raphael field report, 2026-06-13 — 0.16.2/0.16.3 combine ALSO did NOT resolve it).** The
    server is confirmed on **`plugin=0.16.3`** (handshake), but `.faust api bosses` is still ~**24 `up` / ~28
    `down`**, the SAME roaming set down (`CHAR_Villager_CursedWanderer`, `CHAR_Villager_Tailor`, `CHAR_Wendigo`,
    etc.). Raphael re-verified correct (pages all 3, accumulates all 52, renders all — confirmed in the diag log).
    So the map-token "combine" is **not** rescuing these bosses on this server either — almost certainly the
    `N=0` case (`Script_BloodAltar_TrackVBloodUnit_Shared` + the VBlood map icon carry no passive position when no
    player is near). **Faust next step is now the one this note already flagged:** resolve VBlood icons by
    `NetworkId`, or replicate `GetVBloodPositionsSystem` (the system the in-game map itself uses), rather than the
    passive blood-altar/icon components. Please run `.faust admin bossdiag` (no filter) and read the
    `Map tokens: N resolved; M rescued` line to confirm `N=0` before that work. **No Raphael change — Raphael
    displays exactly what Faust sends, and currently renders the whole 52-row board faithfully.**
  - **🔬 RAW-WIRE PROOF the `bosses` ENDPOINT (not Raphael) is the gap (2026-06-13, server `plugin=0.16.3`).** To
    settle "is this Raphael not reading the data?": the literal wire was captured. The `down` rows arrive as e.g.
    `[FAUST:boss] guid=1896428751 name=CHAR_Bandit_Bomber_VBlood status=down defeated=0` — **with no `x=`/`z=`/
    `region=`/`hp=` fields at all** (verified across all ~28 down rows: zero carry an `x=`). Raphael's parser is
    `ReadBoss = new(guid, name, status, defeated, Has("x")?GetFloat("x"):NaN, …)` — it reads `status` and the
    coords exactly as transmitted; there is nothing to "fail to read" because the fields are absent on the wire.
    So if `bossdiag` shows these V Bloods DO have a resolvable position server-side, then the **`bosses` endpoint is
    not emitting that position** (the combine isn't being applied to, or isn't finding a token for, these guids) —
    a **Faust-side endpoint bug**, not a Raphael read bug. Suggested check: log, inside the `bosses` builder, the
    per-guid result of the Translation/blood-altar-token/map-icon lookup for one known-down guid (e.g.
    `CHAR_Bandit_Bomber_VBlood` = 1896428751) and confirm whether it returns a position there; if it does but the
    row still ships `status=down`, the emit path is dropping it.
...
…rows…
[FAUST:note] cmd=worldscan truncated=1 max=<n>          ; ONLY if THIS query's matches exceeded MaxResults (post-filter; sent before [FAUST:end])
...
- **Rate limited by a cached snapshot.** The full-map scan runs at most once per `[Faust.WorldScan]
  ...
  (no extra scan), and there is zero cost when nobody queries. The snapshot captures the **complete**
  whitelisted, on-map world, so a filter scans every asset and never silently misses a match. `MaxResults`
  bounds only the rows **this query returns** (applied *after* the filter); when a broad result is trimmed, a
  **`[FAUST:note] … truncated=1`** line precedes the end trailer (so Raphael can warn "filter to see more").
  *(Fixed: pre-fix `MaxResults` capped the pre-filter snapshot, so on asset-dense maps a narrow filter could
  miss matching assets past the cap; now the cap is post-filter and the snapshot is complete. Wire unchanged →
  ApiVersion stays 18.)*
...
`heatmapcellsize`, `heatmapmaxcells`, `heatmapretentiondays`, `mapmarkersenabled`, `mapmarkerprefab`.
...
- **Heat map:** a player-position **heat-map viz** consuming `.faust api heatmap`
  ...
  **`api ≥ 19`:** add a **today / this week / this month / all-time** toggle that sends
  `days=1|7|30|0` (same control as the kills board); cap the toggle at the header's
  `retentiondays` (windows past it just sum what's retained). `days=0` is the all-time map.
- **ApiVersion 18 (Faust 0.16.0) — to build, gate each on `api ≥ 18` / its handshake token:**
  - **Boss board** (`bosses` token): a **Boss Status** tab/panel consuming `.faust api bosses`
    ...
    and level. **Render EVERY `status=down` row, regardless of `defeated`** (these have no location):
    `defeated=1` → "defeated / not spawned"; **`defeated=0` → "not spawned / awaiting respawn"** (a pooled
    VBlood nobody has beaten yet). **⚠️ Do not drop `status=down defeated=0` rows** — on a fresh server most
    bosses are exactly that, so dropping them makes the back half of the board (the `down` rows that sort after
    the live ones) appear empty even though Faust sends them. Prettify `name` by `guid`. §B1.
...
- **🟢 §16/§18/§18b — "only half the bosses" — RESOLVED server-side; no Raphael change.** Two distinct causes,
  both now handled in Faust: **(§18)** the on-map cutoff is `[Faust.Bosses] MapLimit` (**default 9000**, live via
  `.faust admin setglobal bossmaplimit=…`) — Raphael already removed its old ±5000 client guard (v0.58.1), keep
  it removed. **(§18b, the real "half the bosses" cause, fixed in Faust 0.16.1)** roaming V Bloods read `down`
  because their position was taken from the stale `LocalToWorld` matrix; the board now reads `Translation`
  first, so roamers report `up` with live coords. **Wire shape unchanged (ApiVersion 18)** — Raphael just
  receives coords for more bosses. Confirm the running server reports **`plugin=0.16.1`** before re-testing.
